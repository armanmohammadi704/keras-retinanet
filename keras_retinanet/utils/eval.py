...
def evaluate(
    ...
    for label in range(generator.num_classes()):
        ...
        for i in range(generator.size()):
            ...
            if len(false_positives_i) == 0:
                false_positives1_i = 0
            else:
                false_positives1_i = int(max(np.cumsum(false_positives_i)))
                
            if len(false_positives_ix) == 0:
                false_positives1_ix = 0
            else:
                false_positives1_ix = int(max(np.cumsum(false_positives_ix)))
             
            if len(true_positives_i) == 0:
                true_positives1_i = 0
            else:
                true_positives1_i = int(max(np.cumsum(true_positives_i)))
                
            if len(true_positives_ix) == 0:
                true_positives1_ix = 0
            else:
                true_positives1_ix = int(max(np.cumsum(true_positives_ix)))
